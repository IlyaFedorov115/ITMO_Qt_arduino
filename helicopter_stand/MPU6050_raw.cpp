...
byte MPU6050_raw::begin(int gyro_config_num, int acc_config_num){
...
byte MPU6050_raw::writeData(byte reg, byte data){
...
byte MPU6050_raw::readData(byte reg) {
  wire->beginTransmission(address);
  wire->write(reg);
...
byte MPU6050_raw::setGyroConfig(int config_num){
  byte status;
  switch(config_num){
...
byte MPU6050_raw::setAccConfig(int config_num){
  byte status;
  switch(config_num){
    case 0: // range = +- 2 g
...
void MPU6050_raw::setGyroOffsets(float x, float y, float z){
  gyroXoffset = x;
  gyroYoffset = y;
  gyroZoffset = z;
}

void MPU6050_raw::setAccOffsets(float x, float y, float z){
  accXoffset = x;
  accYoffset = y;
  accZoffset = z;
}

void MPU6050_raw::setFilterGyroCoef(float gyro_coeff){
...
void MPU6050_raw::setFilterAccCoef(float acc_coeff){
  setFilterGyroCoef(1.0-acc_coeff);
}

/* CALC OFFSET */

void MPU6050_raw::calcOffsets(bool is_calc_gyro, bool is_calc_acc){
...
void MPU6050_raw::update(){
  ...
  // Correctly wrap X and Y angles (special thanks to Edgar Bonet!) https://github.com/gabriel-milan/TinyMPU6050/issues/6
