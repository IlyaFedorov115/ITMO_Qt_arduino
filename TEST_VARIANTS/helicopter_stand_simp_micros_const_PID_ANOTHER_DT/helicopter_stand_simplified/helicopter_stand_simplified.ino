...
/*
          ========================== ВЕРСИЯ С ДОБАВЛЕНИЕМ ОТСЧЕТА ВРЕМЕНИ ПО MICROS И ДОБАВЛЕНИЕМ КОНСТАНТНОГО ШАГА ================================
*/
...
struct TIMERS_STRUCT {
  float elapsedTimeSecs = 0.001;
  float elapsedTimeMsecs = 0.01;
  float elapsedTimeMicros = 10;
  float currTimeMicros = 0;
  float currTimeMillis = 0;
  float timePrevMicros = 0;
  float timePrevMillis = 0;

  void startTimers() {
    timePrevMicros = micros();
    timePrevMillis = millis();
  }

  void updateElapsed() {              // for every loop, not for millis
    timePrevMicros = currTimeMicros;
    currTimeMicros = micros();
    elapsedTimeMicros = currTimeMicros - timePrevMicros;

    timePrevMillis = currTimeMillis;
    currTimeMillis = millis();
    elapsedTimeMsecs = currTimeMillis - timePrevMillis;
  }

  void calcElapsedSecFromMil() { elapsedTimeSecs = elapsedTimeMsecs * 0.001; }
  void calcElapsedSecFromMicro() { elapsedTimeSecs = elapsedTimeMicros / 1000000.0; }

} timersController;


const float rad_to_deg = 180/3.141592654;
float pwmSend, error, previous_error;
float elapsedTime;                            /* used for PID calculation and angles, in seconds */
float elapsedTimeAngle;
...
void setup() {
  ...
  timersController.elapsedTimeSecs = 0.003;
...
void loop() {
  ...
  /* When start called startTimer to prepare prevTime */

  timersController.currTimeMicros = micros();
  timersController.elapsedTimeMicros = timersController.currTimeMicros - timersController.timePrevMicros;

  elapsedTimeAngle = timersController.elapsedTimeMicros / 1000000.0;
  /* ============ READ ANGLE FROM MPU ============= */
  getAngle();
  getAngleFilt(); 
   
  Serial.print("A_el: "); Serial.println(elapsedTimeAngle*1000);

  timersController.currTimeMicros = micros();
  timersController.elapsedTimeMicros = timersController.currTimeMicros - timersController.timePrevMicros;

  if (timersController.elapsedTimeMicros >= EXPR_VARS::stepDtMicroSec)
{
  elapsedTime = timersController.elapsedTimeMicros / 1000000.0;
  timersController.timePrevMicros = timersController.currTimeMicros;

  Serial.print("\nmil_el: "); Serial.print(timersController.elapsedTimeMsecs); Serial.print("mic_el: "); Serial.print(timersController.elapsedTimeMicros); 
  Serial.print("el: "); Serial.print(elapsedTime*1000.0);Serial.println();


  // ======= angle from +40 to 0, or from -40 to 0
  if (USE_FILT_ANGLE){
  ...
  pwmSend = throttle + EXPR_VARS::control_signal;

  // saturation
  if (pwmSend < PWM_SEND_MIN) {
    pwmSend = PWM_SEND_MIN;
  } else if (pwmSend > PWM_SEND_MAX) {
    pwmSend = PWM_SEND_MAX;
  }
 


  /* ==== send PWM ==== */
  bldcEsc.writeMicroseconds(pwmSend);
...
// =======================================================  MATHEMATICS ======================================================= //
void PID_Step(double error_, double dt) // dt -  sec
{
  if (dt < 0.0001) return;
...
inline void getAngle()
{
 ...
 // Acceleration_angle[1] = atan(-1*(Acc_rawX/16384.0)/sqrt(pow((Acc_rawY/16384.0),2) + pow((Acc_rawZ/16384.0),2)))*rad_to_deg; /*---Y---*/
  ...
  Gyro_angle[1] = Gyro_angle[1] + Gyro_rawYf * elapsedTimeAngle;    /*---Y---*/
...
void getAngleFilt()
{
  ...
  //Gyro_angle_filt[1] = Gyro_angle_filt[1] + Gyro_rawYf * elapsedTimeAngle;

  // just filt only angle without filt raw accel
...
// send floats in bytes view. Format: ## data(5 or 5 + pid(3)) 
void debugByte()
{
...
/*
inline void prepareRawAngleData()
{
  Wire.beginTransmission(0x68);
  Wire.write(0x3B);                     //Ask for the 0x3B register- correspond to AcX
  Wire.endTransmission(false);
