...
void loop() {
 ...
 // float error = -kalAngleY;   
  ...
  #else
  errorDiff = gyroYrate;  
  #endif// errorDiff = -Gyro_rawYf; filterGyro.filter(errorDiff)
  ...
  /* ======================== БЕЗОПАСНОСТЬ ================ */
//if (count <= 0){  
  if (pwm > PWM_SAT_MAX) pwm = PWM_SAT_MAX;
  ...
  /* determine pwm */
  //pwm = determineU(pwm, allTime);
  if (pwm > PWM_SAT_MAX) pwm = PWM_SAT_MAX;
  if (pwm < MIN_PWM) pwm = MIN_PWM;
  
  servo.writeMicroseconds(pwm);

  float what2print = oldPwm;      /// pwm 
  //float what2print = targetAngle;      /// angle
  float actualAngle = 90 - (error + targetAngle);


  /* change target angle */
  targetAngle = determineAngle(allTime);

  /* error | filt_err | P | I | D | pwm | dt */
  ///*
  Serial.print(error); Serial.print(','); 
  Serial.print(what2print); Serial.print(',');
  Serial.print(error * kp); Serial.print(',');
  Serial.print(errorInt * ki); Serial.print(','); 
  Serial.print(errorDiff * kd); Serial.print(',');
  Serial.print(pwm); Serial.print(','); 
  Serial.println(actualAngle); //allTime
  //Serial.println(String(dt, 6)); 
  //Serial.printlm(oldPwm);
  //*/
