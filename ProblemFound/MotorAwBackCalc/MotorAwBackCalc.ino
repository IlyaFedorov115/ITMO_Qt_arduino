...
void loop() {
 ...
 // float error = -kalAngleY; 
  ...
  #else
    calculateAngles();
    error = -kalAngleY - targetAngle; // -49 -> 49 error - 20 want = 29 error
  #endif
  ...
  /* ============== PID LOGIC =================== */
  float err4Int = error - (1.0/tauSat)*errorSat;
  errorInt += err4Int * dt;
  ...
  /* ========= SATURATION ============= */
  uint16_t pwmBeforeSat = pwm;
  if (pwm > PWM_SAT_MAX) pwm = PWM_SAT_MAX;
  errorSat = pwmBeforeSat - pwm; // Check after saturation
  ...
  float oldPwm = pwm; /* DRAWN PWM and angle */
  pwm = determineU(pwm, allTime);   /* determine pwm */
  if (pwm > PWM_SAT_MAX) pwm = PWM_SAT_MAX;
  if (pwm < MIN_PWM) pwm = MIN_PWM;
  ...
  Serial.println(String(actualAngle, 6)); 
  //*/
